...
We find that the results coincide. Note that the values provided by `summary()` are rounded to two decimal places. Can you do so using <tt>R</tt>?
...
+ Create the vectors <tt>cs</tt> (the class sizea) and <tt>ts</tt> (the test scorea), containing the obeservations above.  
...
The vectors <tt>cs</tt>, <tt>ts</tt> and the model object <tt>mod</tt> from previous exercises are availabe in the working environment.
...
The vectors <tt>cs</tt>, <tt>ts</tt> as well as the model object <tt>mod_ni</tt> from the previous exercise are availabe in your working environment.
...
The matrix <tt>coef</tt> from the previous exercise is availabe in your working environment.
...
It mesaures the size of an average residual which is an estimate of the magnitude of a typical regression error.
...
The model object <tt>mod</tt> is available in your workspace.
...
#### 16. Testing Two Null Hypotheses Individually {-}

Now consider again the estimated regression line

$$ \widehat{TestScore} = \underset{(23.9606)}{567.43} - \underset{(0.8536)}{7.15} \times STR, \, R^2 = 0.8976, \, SER=15.19 $$

with standard errors in parentheses.

+ Compute the $p$-value for a $t$-test of the hypothesis that the intercept is zero. Save the result to <tt>p_int</tt>
+ Compute the $p$-value for a $t-$test of the hypothesis that the coefficient of $cs$ is zero. Save the result to <tt>p_cs</tt>

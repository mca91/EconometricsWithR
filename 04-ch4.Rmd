...
A researcher wants to analyse the relationship between class size and pupils' average test score. Therefore he measures both variables in 10 different classes and ends up the following results:
...
+ Create the vectors `cs` (for class size) and `ts` (for test score), containing the obeservations above.  
...
The vectors <tt>cs</tt> and <tt>ts</tt> are available in the working environment (you can check this: type their names to the console and then press enter or click on the button "run").
...
Use <tt>R</tt> functions presented in this chapter: <tt>mean()</tt>, <tt>sd()</tt>, <tt>cov()</tt>, <tt>cor()</tt> and <tt>var()</tt>
...
+ Use <tt>lm()</tt> to estimate the regression model $$ts_i = \beta_0 + \beta_1 cs_i + u_i.$$
...
Now add a regression line to the scatter plot considered a few exercises before. You are provided with the code for the scatter plot in <tt>script.R</tt>
...
#### Summary of a Model Object {-}

Now read out and store some information contained in the output produced by <tt>summary()</tt>:
...
+ Use the help function on <tt>lm</tt> (<tt>?lm</tt>) to find out how to specify the <tt>formula</tt> argument for a regression of `ts` solely on `cs`, i.e. a regression without intercept

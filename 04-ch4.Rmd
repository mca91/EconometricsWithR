...
Use the <tt>R</tt> functions presented in this chapter: <tt>mean()</tt>, <tt>sd()</tt>, <tt>cov()</tt>, <tt>cor()</tt> and <tt>var()</tt>
...
<div data-datacamp-exercise data-lang="r">
  ...
  test_output_contains("lm(ts ~ cs)")
  test_output_contains("summary(lm(ts ~ cs))")
  </code>
...
Now add a regression line to the scatter plot from a few exercises before. You are provided with the code for the scatter plot in <tt>script.R</tt>
...
+ Assign the output of <tt>summary(mod)</tt> to the variable 's'
...
<tt>summary()</tt> provides information on the statistical significance of the estimated coefficients.
...
Remember: an entry of a named list can be accessed with the <tt>$</tt> operator.

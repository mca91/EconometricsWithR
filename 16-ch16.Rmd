...
It is straightforward to estimate VAR models in `r ttcode("R")`. A feasible approach is to simply use `r ttcode("lm()")` for estimation of the individual equations. Furthermore, the `r ttcode("R")` package `r ttcode("vars")` provides standard tools for estimation, diagnostic testing and prediction using this type of models.  
...
Notice that `r ttcode("VAR()")` returns a list of `lm` objects which can be passed to the usual function, for example `r ttcode("summary()")` and so it is straightforward to obtain model statistics for the individual equations.
...
It is fairly easy to obtain differences of time series in `r ttcode("R")`. For example, the function `r ttcode("diff()")` returns suitably lagged and iterated differences of numeric vectors, matrices and time series objects of the class `r ttcode("ts")`.
...
`r ttcode("R")` functions for cointegration analysis are implemented in the package `r ttcode("urca")`. 

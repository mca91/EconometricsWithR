...
All empirical applications use data on U.S. macroeconomic indicators like Gross Domestic Product (GDP) and the unemployment rate.
...
USMacroSWQ <- read.xlsx2("Data/us_macro_quarterly.xlsx",
                         sheetIndex = 1,
                         header = T,
...
GDP <- xts(USMacroSWQ$GDPC96, as.yearqtr(USMacroSWQ$X., format = "%Y:0%q"))["1960::2013"]
...
chart.TimeSeries(GDPGrowth,
                 ...
                 ylab = "Percent at an annual rate",
...
USUnemp <- xts(USMacroSWQ$UNRATE, 
                    as.yearqtr(USMacroSWQ$X., format = "%Y:0%q"))["1960::2013"]

DollarPoundFX <- xts(USMacroSWQ$EXUSUK, 
                     as.yearqtr(USMacroSWQ$X., format = "%Y:0%q"))["1960::2013"]
  
JPIndProd <- xts(log(USMacroSWQ$JAPAN_IP), 
                 as.yearqtr(USMacroSWQ$X., format = "%Y:0%q"))["1960::2013"]
...
chart.TimeSeries(DollarPoundFX, 
                 date.format = "%Y", 
                 minor.ticks = F,
                 col = "steelblue",
...
SReturns <- read.xlsx2("Data/Stock_Returns_1931_2002.xlsx",
                         sheetIndex = 1,
                         colIndex = 1:4,
                         header = T,
...
2. **Relying on an information criterion**: 
    ...
    The circumvent the issue of producing too large models, one may choose the lag order that minimizes one of the following two information criterea:
...
chart.TimeSeries(merge(USUnemp, JPIndProd),
                 date.format = "%Y", 
                 minor.ticks = F,
...
The optimal lag length $p$ can be estimated using information criterea. Notice that in the regression \@ref(eq:ADFreg1), $p=0$ (that is no lags of $\Delta Y_t$ are used as regressors) corresponds to a simple AR($1$).

...
\begin{align*}
  Y_t =& \, (\rho - 1)c + \rho Y_{t-1} + u_t,
\end{align*}
$t=1,\dots,n$ where $N$ and $n$ are large numbers, $c$ is a constant and $u$ is a zero mean error term. Notice that we obtain a random walk without drift under the null of $\rho=1$.
...
\begin{align*}
  Y_t =& \, c + b (\rho-1) t + \rho Y_{t-1} + u_t
\end{align*}
and estimate
\begin{align*}
...
RWD <- ts(replicate(n = N, (rho-1) * drift + AR1(rho)))
...
RWDT <- ts(replicate(n = N, drift + (rho-1) * trend + AR1(rho)))

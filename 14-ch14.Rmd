...
**Instructions:**
  ...
  + Write a for loop which ...
    ...
    + Generate random samples from an Ar(1) for each sample size using the function <tt>arima.sim()</tt>.

    + Estimate the coefficient using the function <tt>ar.ols()</tt>
...
The packages <tt>dynlm()</tt> and <tt>lmtest</tt> have been loaded.
...
**Hints:**
  ...
  + For only one lagged variable, the test on Granger causlality reduces to a simple t-test.
...
To do so consider the two random walks
...
$$y_{1t}=\beta_0 + \beta_1 \times\y_{2t} + u_t, \\qquad .$$
...
**Instructions:**
  ...
  + Proceed as outlined above and save the value You are totally free  However we recommend the following approach:

    1. Write an inner and outer <tt>for()</tt> loop which iterate over all sample sizes and repetitions, respectively.

    2. Inside the inner loop generate two random walks and estimate the model using <tt>lm()</tt>.
  ...
  + For each sample size (that is, for each column in <tt>tstats</tt>) compute the fraction exceeding the absolute value of 1.96, the 5% critical value of the standard normal distribution. What do you observe?
...
**Hints:**
  ...
  + You can use <tt>apply</tt> to compute the fraction for each column. 
...
#### 6. Stochastic Trends - The ADF Test {-}

Consider the daily closing prices of the DAX for the time period 1991-1998 available in the dataset <tt>EuStockMarkets</tt>. Your task in this exercise is check 

The packages <tt>dynlm</tt>, <tt>lmtest</tt> and <tt>urca</tt> have been loaded.
      
**Instructions:**

  + Estimate the model necessary to conduct the augmented Dickey-Fuller test.

  + Test with the logical comparison whether the test rejects at the 1%, 5% and 10% level, respectively. The critical values for the ADF test are already available for you as <tt>cval</tt>.
...
#### 7. Nonstationarity - Breaks {-}

Consider the 
      
**Instructions:**

  + Based on the plot do you ..? If so, where? 

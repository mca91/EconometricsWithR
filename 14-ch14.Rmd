...
When dealing with time series data in <tt>R</tt> it is preferable to work with time-series objects that keep track of the frequency of the data and are extensible. In what follows we will use objects of the class <tt>xts</tt>, see `?xts`. Since the data in `USMacroSWQ` are in quarterly frequency we convert the first column to `yearqtr` format before generating the <tt>xts</tt> object `GDP`.
...
The function `forecast()` from the <tt>forecast</tt> package has some useful features for forecasting of time series data.
...
After computing robust standard errors, we gather the results in a table generated by <tt>stargazer()</tt>.
...
Notice that from the definition of `BIC()`, for ADL models with $p=q$ it follows that <tt>p</tt> reports the number of estimated coefficients *excluding* the intercept. Thus the lag order is obtained by dividing <tt>p</tt> by 2.
...
\end{align*}
which is convenient since the corresponding test statistic is reported by many relevant <tt>R</tt> functions.^[The $t$-statistic of the Dickey-Fuller test is computed using homoskedasticity-only standard errors since under the null hypothesis, the usual $t$-statistic is robust to heteroskedasticity.]
...
Key Concept 14.8 states that the critical values for the ADF test in the regressions \@ref(eq:ADFreg1) and \@ref(eq:ADFreg2) can only be determined using simulation. The idea of the simulation study is to simulate a large number of ADF test test statistics and use them to estimate quantiles of their *asymptotic* distribution. This section shows how this is feasible within <tt>R</tt>.
...
We next translate the pseudo-out-of-sample forecasts into an object of class <tt>ts</tt> and plot the real GDP growth rate against the forecasted series.
...
If model (3) would be of any use for predicting excess returns, pseudo-out-of-sample forecasts based on (3) should outperform forecasts of an intercept-only model in terms of the sample RMSFE. We can perform this type of comparison using <tt>R</tt> code in the fashion of the applications of Chapter \@ref(niib).
...
This chapter dealt with introductory topics in time series regression analysis, where variables are generally correlated from one observation to the next, a concept termed serial correlation. At the beginning, several ways of storing and plotting time series data using <tt>R</tt> have been presented and used for informal analysis of economic example data.

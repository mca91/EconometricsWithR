...
where <tt>violent</tt> and <tt>law</tt> denote the violent crime rate (incidents per 100000) and a binary variable for the implementation of a CCW law (1 = yes, 0 = no), respectively.

The packages <tt>AER</tt> and <tt>plm</tt> have been loaded.
      
**Instructions:**
        
  + Is the proposed model a good model? In particular are there variable that vary across

  + Print a summary of the model using robust standard errors. Critically analyze your result.
...
<div data-datacamp-exercise data-lang="r">
  ...
  <code data-type="sct">
  success_msg("Correct! The proposed model has two severe shortcomings: First the model does not account for variables that vary across the entities... Still the coefficient is relatively large in magnitude which is likely due to omitted variable bias.")
  </code>
...
As touched upon the end of the last exercise there is likely an omitted variables bias.. For that reason we incorporate further criminal and sociodemographic variables into our model which then reads as follows

$$\\log(violent_i) = \\beta_1\\times law_i + \\beta_2\\times density_i + \\beta_3\\times income_i + \\beta_4\\times population_i + \\beta_5\\times afam_i + \\beta_6\\times cauc_i + \\beta_7\\times male_i + \\alpha_i + u_i.$$
...
#### 5. Fixed Effects Regression - Estimation {-}

This exercise serves to illustrate that for $T=2$ the "Before and After" comparison is 

The package <tt>AER</tt> has been loaded.
      
**Instructions:**
        
  + Estimate the coefficients of the model by incorporating dummies for each entity.

  + 

  + Estimate the coefficients of the model using the entity-demeaned data.

  + Verify that both procedures numerically yield the same estimates.
...
<div data-datacamp-exercise data-lang="r">
  ...
  <code data-type="sample-code">
  # estimate the model using dummies for each entity
  
  
  # 
  
  
  # estimate the model using the entity-demeaned data
  
  
  # verify that both estimates are numerically identi

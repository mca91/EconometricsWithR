...
In this exercise, you will generate simulation evidence for the bias of $\\hat{\\beta}_0$ in the model $$Y_i = \\beta_0 + X_i \\beta_1 + u_i$$ if the population regression function is $$Y_i = X_i^2.$$
...
+ Generate 1000 OLS estimates of $\\beta_0$ in the model above using a <tt>for()</tt> loop where $X_i \\sim \\mathcal{U}[-5,5]$, $u_i \\sim \\mathcal{N}(0,1)$. Save the estimates in <tt>beta_hats</tt>.

+ Compare the sample mean of the estimates to the true parameter using the <tt>==</tt> operator.

<!--html_preserve-->

<div data-datacamp-exercise data-lang="r" height = "800">
...
+ You can generate random numbers from a uniform distribution using <tt>runif()</tt>, see `?runif`.

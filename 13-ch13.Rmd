...
`r ttcode("is.na(STAR[1, ])")` returns a logical vector with `r ttcode("TRUE")` at positions that correspond to `r ttcode(<"NA>")` entries for the first observation. The `r ttcode("!")` operator is used to invert the result such that we obtain only non-`r ttcode("<NA>")` entries for the first observations.
...
Following the book, we augmente the regression model \@ref(eq:starpopreg) by different sets of regressors for two reasons: 
...
coeftest(gradeK1, vcov. = vcovHC(gradeK1, type = "HC1"))
coeftest(gradeK2, vcov. = vcovHC(gradeK2, type = "HC1"))[1:4, ]
coeftest(gradeK3, vcov. = vcovHC(gradeK3, type = "HC1"))[1:7, ]
```
...
What if we a SRDD instead, thereby ignoring the fact that treatment is not perfectly determined by the cutoff in $W$? We may get an impression of the consequences by estimating an SRDD using the previously simulated data.

...
This is a joint null hypothesis with $r-1$ restrictions so it can be tested using the $F$-test presented in Chapter 7. Remember that the function <tt>linearHypothesis()</tt> can used to conduct such tests. If, for example, we would like to test the null hypothesis of a linear model against the alternative of a polynomial of a maximal degree $r=3$, we could simply do the following:
...
There is no unambiguous guideline how to choose $r$ in step one. However as Stock and Watson (2014) point out, economic data is often smooth such that it is appropriate to choose small orders like $2$, $3$, or $4$.

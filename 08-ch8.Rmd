...
Here, income and test scores are positively related: school districts with above average income tend to achieve above average test scores. Does a linear regression fuction model the data adequately? Let us plot the data and add a linear regression line.
...
A cubic model for instance can be estimated in the same way as the quadratic model; we just have to use a polynomial of degree $r=3$ in  <tt>income</tt>. This conveniently done using the function `r ttcode("poly()")`.
...
This is a joint null hypothesis with $r-1$ restrictions so it can be tested using the $F$-test presented in Chapter \@ref(htaciimr). <tt>linearHypothesis()</tt> can be used to conduct such tests. For example, we may test the null of a linear model against the alternative of a polynomial of a maximal degree $r=3$ as follows.
...
The reported standard errors have changed. Furthermore, the coefficient for `income^3` is now significant at the $5\%$ level. This means we reject the hypothesis that the regression function is quadratic against the alternative that it is cubic. Furthermore, we can also test if the coefficients for <tt>income^2</tt> and <tt>income^3</tt> are jointly significant using a robust version of the $F$-test. 
...
The estimated regression function is $$\widehat{\ln(TestScore)} = 6.439 + 0.00284 \times income.$$ An increase in district income by $\$1000$ is exptected to increase test scores by $100\times 0.00284 \% = 0.284\%$. 
...
# compute the expected difference
Y_hat_matrix <- matrix(exp(Y_hat), nrow = 2, byrow = TRUE)
Y_hat_matrix[, 2] - Y_hat_matrix[, 1]
```

We use `r ttcode("exp()")` to transform the fitted values back to the original scale in order to compute expected changes in test scores. The log-linear model states that an income increase from $\$10000$ to $\$11000$ increases test scores by an expected amount of $1.83$ points. When district income increases from $\$40000$ to $\$41000$, the expected increase in test scores is only about $2.00$ points.
...
We now reproduce Figure 8.5 of the book and compute expected changes in test scores due to changes in district income considered in cases I and II.
...
As for the log-linear model, we transform the fitted values using `r ttcode("exp()")`. The log-log model states that an income increase from $\$10000$ to $\$11000$ increases test scores by an expected amount of $3.40$ points while for a increase in district income from $\$40000$ to $\$41000$, the expected increase in test scores is $0.95$ points.
...
The following subsections discuss these cases briefly and demonstrate how to perform such regressions in <tt>R</tt>.
...
There are several ways to add the interaction term to the <tt>formula</tt> argument when using <tt>lm()</tt> but the most intuitive way is to use <tt>HiEL * HiSTR</tt>.^[Appending <tt>HiEL * HiSTR</tt> to the formula will add <tt>HiEL</tt>, <tt>HiSTR</tt> *and* their interaction as regressors while <tt>HiEL:HiSTR</tt> only adds the interaction term.]
...
However, the output of <tt>summary()</tt> indicates that the difference of the effect for the median and the $75\%$ quantile is not statistically significant. $H_0: \beta_3 = 0$ cannot be rejected at the $5\%$ level of significance (the $p$-value is) 

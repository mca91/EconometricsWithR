...
$$\\widehat{medv_i} = 34.554 - 0.95\\times lstat_i + u_i.$$

with <tt>medv</tt> (median house value) and <tt>lstat</tt> (percent of households with low socioeconomic status) being variables from the already known <tt>Boston</tt> data set.

The <tt>lm()</tt>-object of the above model is available as <tt>mod</tt> in your working environment. The package <tt>MASS</tt> has been loaded.

**Instructions:**

  + Compute the correlation between <tt>medv</tt> and <tt>lstat</tt>.
  
  + Plot <tt>medv</tt> against <tt>lstat</tt> and add the regression line of the estimated model. Especially with respect to the previous result what do you notice?
...
<div data-datacamp-exercise data-lang="r">
  ...
  success_msg("Correct! Although the correlation coefficient suggests that medv and lstat are negatively linearly dependent, the relationship between them is clearly nonlinear. Hence a simple linear regression specification is inappropriate here.")
...
**Hints:**
  ...
  + As usual you can use <tt>plot()</tt> and <tt>abline()</tt> to visualize regression results.
...
In the previous exercise we saw that the correlation coefficient is not able to properly capture the functional form of a regression model in that it suggested by construction a linear form, whereas the true functional form seems to be nonlinear (e.g. exponential).

As an alternative consider the following nonlinear model specification
...
**Instructions:**
  ...
  + Conduct the regression from above and assign it to <tt>log_mod</tt>.
...
**Hints:**
  ...
  + Use <tt>lm()</tt> to conduct a regression.
...
**Instructions:**
  ...
  + Determine the optimal order of the polylog model by the sequential testing approach. Assume a maximum polynomial order of $r=4$ and a significance level of $\\alpha=0.05$. You are more or less free to solve this exercise as long as you use a <tt>for()</tt> loop. We recommend the following approach:
    1. Estimate a model, say <tt>mod</tt>, which starts with the highest polynomial order.
    2. Save the p-value (with robust standard errors) of the relevant parameter and compare it with the significance level $\\alpha$.
  ...
  + Extract the $R^2$ of the selected model and assign it to the variable <tt>R2</tt>.
...
**Hints:**
  ...
  + You can use <tt>poly()</tt> inside of <tt>lm()</tt> as a generic way of incorporating higher orders of a certain variable in the model. Besides the variable, you have to specify the degree of the polynomial via the argument <tt>degree</tt> and set <tt>raw = TRUE</tt>.

  + You can use <tt>coeftest()</tt> together with the argument <tt>vcov</tt> to obtain p-values (with robust standard errors). Use the structure of the resulting object to extract the relevant p-value.

  + Use an <tt>if()</tt> statement to check whether the condition for no rejection is fulfilled and stop the <tt>for()</tt> loop with the help of <tt>break</tt> otherwise.

  + To obtain the $R^2$, you can use <tt>summary()</tt> and extract it via <tt>$r.squared</tt>.
...
#### 4. Estimated Effect of a Unit Change {-}

Reconsider the polylog model from the previous exercise that was selected by the sequential testing approach. As this model is logarithmic and of quadratic form, we cannot simply read off the estimated effect of a unit change in <tt>lstat</tt> from the coefficient summary. Instead we have to compute it manually. To do so assume in the following that we are interested in the effect of an increase in <tt>lstat</tt> from 10% to 11%.

The selected polylog model is available as <tt>mod_pl</tt> in your working environment. The package <tt>MASS</tt> has been loaded.
      
**Instructions:**
        
  + Arrange the relevant data points in a data frame.

  + Use the new values to predict the corresponding values of <tt>medv</tt>.

  + Compute the expected effect with the help of the function <tt>diff()</tt>.
...
- Note that the column name of the data points in the data frame has to coincide with the name of the regressor.

- You can use <tt>predict()</tt> together with the new data points to obtain the relevant values of <tt>medv</tt>.

- <tt>diff()</tt> expects a vector and computes the differences between all entries of this vector.
...
$$medv_i=\\beta_0+\\beta_1\\times chas_i+\\beta_2\\times old_i+\\beta_3(chas_i\\times old_i)+u_i$$

where $chas_i$ and $old_i$ are dummy variables. The first one takes the value $1$, if the Charles River (a short river in the proximity of Boston) passes through suburb $i$ and $0$ otherwise. The latter accounts for a high proportion of old buildings and is constructed as follows
...
**Instructions:**
  ...
  + Append the binary variable <tt>old</tt> to the dataset <tt>Boston</tt>.

  + Conduct the regression from above and assign it to <tt>mod_bb</tt>.

  + Obtain a coefficient summary of the model. How can you interpret the coefficient $\\hat{\\beta}_3$?
...
**Hints:**
  ...
  + Use the logical operator <tt>>=</tt> to create a Boolean vector which then can be transformed to a numeric one via <tt>as.numeric()</tt>.

  + Inside of <tt>lm()</tt> there are two possibilities to incorporate interaction terms:
      1. <tt>Var1*Var2</tt> to add <tt>Var1</tt>, <tt>Var2</tt> and the corresponding interaction term at once.  
      2. <tt>Var1:Var2</tt> to manually add the interaction term (which of course requires to add the remaining terms manually as well)

  + Use <tt>summary()</tt> to get a comprehensive overview of your model estimation.
...
with $old_i$ defined as in the previous exercise and $indus_i$ being the proportion of non-retail business acres per suburb.

The variable <tt>old</tt> is already available in the dataset. The package <tt>MASS</tt> has been loaded.
      
**Instructions:**
        
  + Conduct the regression from above and assign it to <tt>mod_bc</tt>.
...
<div data-datacamp-exercise data-lang="r">
  ...
  test_or(ex() %>% check_function("plot") %>% {
    ...
    ex() %>% override_solution("plot(Boston$indus, Boston$medv)") %>% check_function("plot") %>% {
          check_arg(., "x") %>% check_equal()
          check_arg(., "y") %>% check_equal()
        },
    ex() %>% override_solution("plot(Boston$medv ~ Boston$indus)") %>% check_function("plot") %>%
          check_arg("formula") %>% check_equal()
  )
  test_or(ex() %>% check_function("abline") %>% {
          check_arg(., "a") %>% check_equal()
          check_arg(., "b") %>% check_equal()
        },
    ex() %>% override_solution("abline(coef = c(params[1], params[2]))") %>% check_function("abline") %>%
          check_arg("coef") %>% check_equal()
  )
  test_or(ex() %>% check_function("abline") %>% {
          check_arg(., "a") %>% check_equal()
          check_arg(., "b") %>% check_equal()
        },
    ex() %>% override_solution("abline(coef = c(params[1] + params[3], params[2] + params[4]))") %>% check_function("abline") %>%
          check_arg("coef") %>% check_equal()
  )
  success_msg("Correct!")
  </code>
...
**Hints:**
  ...
  + You can use the structure of <tt>mod_bc</tt> or the function <tt>coef()</tt> to extract the estimated coefficients.

  + Besides passing a <tt>lm()</tt>-object to <tt>abline()</tt>, one can also manually specify intercept and slope with the arguments <tt>a</tt> and <tt>b</tt>, respectively.

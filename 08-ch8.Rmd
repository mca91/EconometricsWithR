...
$$medv_i = \\beta_0 + \\beta_1\\times lstat_i + u_i.$$
...
In the previous exercise we saw that the correlation coefficient is not able to properly capture the functional form of a regression model in that it suggested a linear form, whereas the true functional form seems to be nonlinear (e.g. exponential).
...
**Instructions:**
  ...
  + Conduct the regression from above and assign the results to <tt>log_mod</tt>.
...
**Instructions:**
  ...
  + Determine the optimal order of the polylog model by the sequential testing approach. Assume a maximum polynomial order of $r=4$. You are more or less free to choose a way to solve this as long as you use a <tt>for()</tt> loop. We recommend the following approach:
...
**Hints:**
  ...
  + As usual, you can use <tt>coeftest()</tt> to obtain p-values (with robust standard errors). Use the structure of the resulting object to extract the relevant p-value.
...
$$medv_i=\\beta_0+\\beta_1\\times chas_i+\\beta_2\\times HighIndus_i+\\beta_3(chas_i\\times HighIndus_i)+u_i$$

\\begin{align}
HighIndus_i =  \\begin{cases}
...
**Instructions:**
  ...
  + Add the binary variable <tt>HighIndus</tt> to the dataset <tt>Boston</tt>.

  + Conduct the regression from above and assign it to <tt>mod_bb</tt>. How can you interpret
...
**Hints:**
  ...
  + Inside of <tt>lm()</tt> you have to two possibilities

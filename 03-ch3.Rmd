...
Using `r ttcode("R")` we can get a notion that $s_y$ is a consistent estimator for $\sigma_Y$, that is
...
Using `r ttcode("R")`we can show that if $\mu_{Y,0}$ equals the true value, that is the null hypothesis is true, \@ref(eq:tstat) is approximately distributed $N(0,1)$ when $n$ is large.
...
The \\textit{$p$-value} is the probability that, in repeated sampling under the same conditions, meaning i.i.d. sampling, the same null hypothesis and the same sample size, a test statistic is observed that provides just as much evidence against the null hypothesis as the test statistic actually observed.\\newline
...
A $95\\%$ confidence interval for $\\mu_Y$ is a `r ttcode("random variable")` that contains the true $\\mu_Y$ in $95\\%$ of all possible random samples. When $n$ is large we can use the normal approximation. Then, $99\\%$, $95\\%$, $90\\%$ confidence intervals are
...
There are several ways to import the `r ttcode(".xlsx")`-files into `r ttcode("R")`. Our suggestion is the function `r ttcode("read_excel()")` from the `r ttcode("readxl")` package. The package is not part of `r ttcode("R")`'s base version and has to be installed manually.
...
Next, install and load the package `r ttcode("dyplr")`. This package provides some handy functions that simplify data wrangling a lot. It makes use of the `r ttcode("%>%")` operator.
...
The next code chunk uses the function `r ttcode("mvnorm()")` from package `r ttcode("MASS")` to generate bivariate sample data with different degree of correlation. 
